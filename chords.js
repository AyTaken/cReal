...
exports.getNotesChord = function(chord) {
    ...
    if (rqi[0].includes('b')) rqi[0] = chromaSharpNormalization(rqi[0]);
    if (rqi[2].includes('b')) rqi[2] = chromaSharpNormalization(rqi[2]);
...
function parse(chord) {
    ...
    if (chord.includes('/')) {
        for (; point < chord.indexOf('/'); point++) quality.push(chord[point]);
        for (let index = chord.indexOf('/') + 1; index < chord.length; index++)
            inversion.push(chord[index]);
    } else {
        for (; point < chord.length; point++) quality.push(chord[point]);
    }
...
function notes(root, mask, inversion) {
    ...
    else rootIndex = keyboard.indexOf(root + '2');
    ...
    if (inversion != '') {
        ...
        if (index != -1) {
            let dist =
                keyboard.indexOf(chordNotes[index]) - keyboard.indexOf(chordNotes[0]);
